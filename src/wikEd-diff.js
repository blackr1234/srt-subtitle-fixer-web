...
var WikEdDiff = function () {
	...
	this.config = {
		...
		stylesheet:
			// Insert
			".wikEdDiffInsert {" +
			"font-weight: bold; background-color: #aed581; " +
			"color: #222; border-radius: 0.25em; padding: 0.2em 1px; " +
			"} " +
			".wikEdDiffInsertBlank { background-color: #aed581; } " +
			// Delete
			".wikEdDiffDelete {" +
			"font-weight: bold; background-color: #ef9a9a; " +
			"color: #222; border-radius: 0.25em; padding: 0.2em 1px; " +
			"} " +
			".wikEdDiffDeleteBlank { background-color: #ef9a9a; } " +
			// Block
			".wikEdDiffBlock {" +
	...
	this.config.regExp = {
		...
		split: {
			...
			// Split into lines
			line: new RegExp("\\r\\n|\\n|\\r|[" + this.config.regExpNewLinesAll + "]", "g"),
		...
		blankOnlyToken: new RegExp(
			"[^" + this.config.regExpBlanks + this.config.regExpNewLinesAll + this.config.regExpNewParagraph + "]"
		),

		// RegExps for sliding gaps: newlines and space/word breaks
		slideStop: new RegExp("[" + this.config.regExpNewLinesAll + this.config.regExpNewParagraph + "]$"),
		slideBorder: new RegExp("[" + this.config.regExpBlanks + "]$"),

		// RegExps for counting words
		countWords: new RegExp(
			"(\\w+|[_" + this.config.regExpLetters + "])+(['’][_" + this.config.regExpLetters + "]*)*",
			"g"
		),
		...
		clipLine: new RegExp("[" + this.config.regExpNewLinesAll + this.config.regExpNewParagraph + "]+", "g"),
		...
		clipTrimNewLinesLeft: new RegExp(
			"[" + this.config.regExpNewLinesAll + this.config.regExpNewParagraph + "]+$",
			"g"
		),
				...
				if (text.tokens[back].link === null) {
					while (
						...
						if (front !== null) {
							...
							if (regExpSlideBorder.test(text.tokens[front].token) !== gapFrontBlankTest) {
								frontStop = front;
							}
						}
	 ...
	 */
	this.calculateDiff = function (level, recurse, repeating, newStart, oldStart, up, recursionLevel) {
		// Set defaults
		...
		for (var i = 0; i < symbolsLength; i++) {
			...
			if (symbols.token[i].newCount === 1 && symbols.token[i].oldCount === 1) {
				...
				if (newTokenObj.link === null) {
					...
					if (this.config.regExp.blankOnlyToken.test(newTokenObj.token) === true) {
						...
						if (recursionLevel === 0) {
							...
							if (level === "character") {
								...
								var words = (token.match(this.config.regExp.countWords) || []).concat(
			...
			if (fragment !== fragments.length - 3) {
				...
				if (rangeRight === null) {
					for (var j = headings.length - 1; j >= 0; j--) {
						if (headings[j] < textLength - this.config.clipHeadingRight || headings[j] < rangeRightMin) {
							break;
						}
				...
				if (rangeRight === null) {
					for (var j = lines.length - 1; j >= 0; j--) {
						if (lines[j] < textLength - this.config.clipLineRightMax || lines[j] < rangeRightMin) {
							break;
						}
